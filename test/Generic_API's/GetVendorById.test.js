...
it('API Success ',
  async () => {
    const options = {
        path: `/${STAGE}/${VERSION}/api/searchvendors?vendor_id=5`,
    ...
    try {
      ...
      reporter.endStep();
      reporter.testId("API Endpoint-/api/searchvendors?vendor_id=5")
...
it('Wrong vendor id is passed',
  async () => {
    const postData = JSON.stringify({});
    const options = {
        path: `/${STAGE}/${VERSION}/api/searchvendors?vendor_id=@23232323@@@@`,
        ...commonOptionsGETBasicAUTH,
      };
    try {
      reporter.startStep("Values passed:" + JSON.stringify(postData));
      ...
      reporter.endStep();
      reporter.testId("API Endpoint-/api/searchvendors?vendor_id=@23232323@@@@")
      reporter.description("Response message from API:"+JSON.stringify(response))
